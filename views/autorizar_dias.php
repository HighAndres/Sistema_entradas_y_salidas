...
$permisos=$conectar->query("SELECT * FROM entrada_salida WHERE estatus='Pendiente' ORDER BY fecha_creacion DESC");
$nr=$permisos->num_rows;
$arrayr=$permisos->fetch_assoc();

//ARRAY OBTENER ID
if(isset($_REQUEST['folio'])){ 
$qaut=$conectar->query("SELECT * FROM entrada_salida WHERE MD5(concat('".$key."',id_es))='".$_REQUEST['folio']."'");
...
if(isset($_POST['editar'])){
    ...
    $conectar->query("UPDATE entrada_salida SET estatus='$estatus' WHERE id_es= '$id'");
    ...
    header("Location:autorizar_dias.php");
    exit();
}
...
<body>
    ...
    <div class="cont">
        <div class="encabezado">
            <h3>Autorizar permisos de entradas y salidas</h3>
        ...
        <div class="tabla">
            <table id="tablax" class="display table-striped table-bordered dataTable">
                ...
                <tbody>
                    ...
                    <div class="modal fade" id="edit_<?php echo $arrayr['id_es']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="container-fluid">
                                        <form method="POST" action="autorizar_dias.php?id=<?php echo $arrayr['id_es']; ?>">
                    ...
                    <tr>
                        <td>
                            <?php echo strftime("%d/%b/%Y",strtotime($arrayr['fecha_creacion'])); ?>
                        </td>
                        <td>
                            <?php echo $arrayr['user'];?>
                        </td>
                        <?php if($arrayr['fecha_salida']>0 && $arrayr['hora_salida']>0) { ?>
                        <td><?php echo strftime("%d/%b/%Y",strtotime(   $arrayr['fecha_salida'])); ?><br><?php echo date('H:i', strtotime($arrayr['hora_salida'])); ?> </td>
                        <?php }else{ ?>
                        <td> </td>
                        <?php } ?>
                        <?php if($arrayr['fecha_entrada']>0 && $arrayr['hora_entrada']>0) { ?>
                        <td><?php echo strftime("%d/%b/%Y",strtotime(   $arrayr['fecha_entrada'])); ?><br><?php echo date('H:i', strtotime($arrayr['hora_entrada'])); ?> </td>
                        <?php }else{ ?>
                        <td> </td>
                        <?php } ?>
                        <?php if($arrayr['inasistencia_del']>0 && $arrayr['inasistencia_al']>0) { ?>
                        <td>Del: <?php echo strftime("%d/%b/%Y",strtotime(   $arrayr['inasistencia_del'])); ?><br>&nbsp;&nbsp;Al: <?php echo strftime("%d/%b/%Y",strtotime($arrayr['inasistencia_al'])); ?> </td>
                        <?php }else{ ?>
                        <td> </td>
                        <?php } ?>
                        <td><?php echo $arrayr['estatus']; ?></td>
                        <td><a href="#edit_<?php echo $arrayr['id_es']; ?>" class="btn btn-success btn-sm" data-toggle="modal"><i class="fas fa-user-edit"></i></a></td>
                        <td><a href="../pdf/pdf_permiso_dia.php?refd=<?php echo md5($key.$arrayr['id_es']); ?>" target="_blank" class="btn btn-info btn-sm"><i class="fas fa-eye"></i></a></td>
                    </tr>
